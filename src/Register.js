...
app.post('/api/employees', (req, res) => {
    const {
      ...
      ZipCode = null, // Default to null if not provided
      isActive = true, // Default to true if not provided
    } = req.body;
  
    // Validate required fields
    if (!FirstName || !LastName || !Gender || !DateOfBirth || !JobTitle || !Department || !Email || !PhoneNumber) {
      return res.status(400).json({ status: false, message: 'Missing required fields.' });
    }
  
    // Check for valid salary
    if (Salary !== undefined && Salary < 0) {
      return res.status(400).json({ status: false, message: 'Salary must be a non-negative number.' });
    }
  
    const query = `
      INSERT INTO employee (FirstName, LastName, Gender, DateOfBirth, JobTitle, Department, Email, PhoneNumber, Salary, Address, ZipCode, isActive)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `;
  
    db.query(
      query,
      [FirstName, LastName, Gender, DateOfBirth, JobTitle, Department, Email, PhoneNumber, Salary || 0, Address, ZipCode, isActive],
      (err, result) => {
        if (err) {
          console.error('Error inserting employee:', err);
          return res.status(500).json({ status: false, message: 'Error inserting employee.', error: err.message });
        }
        res.status(201).json({ status: true, message: 'Employee added successfully.', data: result });
      }
    );
  });
